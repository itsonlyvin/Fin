...
class _EmpIdPageState extends State<EmpIdPage> {
  ...
  // Get single Employee ID details
  Future<void> getEmployeeIdDetails(String emp) async {
    final url = Uri.parse("${AppConfig.baseUrl}/employee-ids/$emp");
    try {
      final response = await http.get(url);
      if (response.statusCode == 200) {
                              ...
                              children: [
                                ...
                                IconButton(
                                  icon: const Icon(Icons.delete,
                                      color: Colors.red),
                                  onPressed: () =>
                                      deleteEmployeeId(emp['employeeId']),
                                ),
                                IconButton(
                                  icon: const Icon(Icons.info_outline),
                                  tooltip: "Get EmployeeId details",
                                  onPressed: () =>
                                      getEmployeeIdDetails(emp['employeeId']),
                                ),
                                IconButton(
